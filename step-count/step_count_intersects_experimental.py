...
idx = np.argwhere(np.diff(np.sign(average - averagegraph))).flatten()


deletethese = []
w = 0
while w < len(idx):
    if idx[w] < 2000 or idx[w] > 6000:
        deletethese.append(w)
    w = w + 1

idx = np.delete(idx, deletethese)

#the range of the "window" could be bette if made variable
getrid = []
window = 25
k=1
while k < len(idx):
...
idx = np.delete(idx, getrid)

print("This is the number of steps")
print(len(idx))
