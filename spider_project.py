...
while q < len(averageECG) - start:
    if averageECG[q] >= movingavgECG[q]:
...
for i in anxietysignaturesBR:
    anxietysignaturesECG.append(i)

anxietysignatures = [i for i, cnt in Counter(anxietysignaturesECG).items() if cnt > 1]
print(len(anxietysignatures))
print(anxietysignatures)
#plt.plot(averageECG)
#plt.title('Averaged')
#plt.show()
