...
while w < len(averageBR) - start:
    if averageBR[w] >= (movingavgBR[w] - windowBR) and averageBR[w] <= (movingavgBR[w] + windowBR):
...
while q < len(averageECG) - start:
    if averageECG[q] >= (movingavgECG[q] - windowECG) and averageECG[q] <= (movingavgECG[q] + windowECG):
